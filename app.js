const PRESETS = {
  shonen: ["Naruto", "One Piece", "Dragon Ball Z", "Bleach", "My Hero Academia", "Demon Slayer", "Hunter x Hunter", "Fullmetal Alchemist: Brotherhood", "Jujutsu Kaisen"],
  romance: ["Toradora", "Your Lie in April", "Clannad", "Fruits Basket", "Horimiya", "Kaguya-sama: Love is War", "Oregairu", "Ao Haru Ride"],
  starter: ["Cowboy Bebop", "Death Note", "Attack on Titan", "Steins;Gate", "Neon Genesis Evangelion", "Violet Evergarden", "Made in Abyss", "Vinland Saga"]
};
...
async function fetchAniList(title) {
  const query = `query ($search: String) { Page(page: 1, perPage: 1) { media(search: $search, type: ANIME) { title { romaji english } coverImage { large } } } }`;
  try {
    ...
    return { found: true, title: media.title.english || media.title.romaji, image: media.coverImage.large };
  } catch (err) {
...
async function fetchJikan(title) {
  try {
    ...
    if (!item) return null;
    return { found: true, title: item.title_english || item.title, image: item.images.jpg.large_image_url };
  } catch (err) {
...
function renderGrid() {
  ...
  elements.grid.className = `grid-container grid-${state.gridMode === 'auto' ? 'auto' : state.gridMode + 'x' + state.gridMode}`;
  elements.grid.innerHTML = "";

  const totalTiles = state.gridMode === "auto" ? state.entries.length : Math.pow(parseInt(state.gridMode), 2);
  ...
  for (let i = 0; i < totalTiles; i++) {
    ...
    if (!result) {
      ...
      // If direct URL fails (CORS error or 403 on MAL), try codetabs proxy
      img.onerror = () => {
        if (!img.dataset.triedProxy) {
          ...
          // If all fail, display error state
          tile.classList.add("not-found");
...
function init() {
  ...
  elements.presetShonen.onclick = () => { elements.input.value = PRESETS.shonen.join("\n"); handleGenerate(); };
  elements.presetRomance.onclick = () => { elements.input.value = PRESETS.romance.join("\n"); handleGenerate(); };
  elements.presetStarter.onclick = () => { elements.input.value = PRESETS.starter.join("\n"); handleGenerate(); };
